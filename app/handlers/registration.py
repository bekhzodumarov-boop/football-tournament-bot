...
async def reg_name(message: Message, state: FSMContext):
    ...
    await message.answer(
        f"👍 Отлично, <b>{name}</b>!\n\n"
        "Шаг 2/3: Выбери свою позицию на поле:",
...
async def reg_position(call: CallbackQuery, state: FSMContext):
    ...
    await call.message.edit_text(
        f"✅ Позиция: {pos_label}\n\n"
        "Шаг 3/3: Оцени свой уровень игры от 1 до 10\n"
...
async def reg_self_rating(call: CallbackQuery, state: FSMContext, session: AsyncSession):
    ...
    new_player = Player(
        ...
        rating=round(self_rating * 0.85, 1),  # провизорный рейтинг = самооценка × 0.85
    ...
    await call.message.edit_text(
        ...
        "Нажми /start чтобы перейти в главное меню:",
